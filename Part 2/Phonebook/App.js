...
const App = () => {
  ...
  const deleteNumber = (event) => {
    ...
    if (window.confirm(`Delete number?`)) {
      ...
      let stringDel = deletePerson.toString()
        console.log(idnumber)
        console.log(deletePerson)

      axios
        .delete('http://localhost:3001/persons/'+(idnumber))
        .then(response => {
  ...
  const clickName = (event)  => {
    ...
    console.log(newPersons.name)
      ...
      if (existingNumber) {
          ...
          return  }
      if (existingId) {
          alert("Duplicate I.D, ID = Phonenumber")
          newPersons.id = persons.lenght + 10
      }

      if (existingName) {
          console.log(newPersons.id)
          console.log(existingName.id)
          (window.confirm(newPersons.name + ` is already on the phonebook, replace old number with new one?`)) 
            axios
              .put('http://localhost:3001/persons/'+(existingName.id),newPersons)
              .then(response => {
                console.log(response)
                {
                axios
